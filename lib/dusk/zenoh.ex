defmodule Dusk.Zenoh do
  ...
  def init(opts) do
    ...
    key = Keyword.get(opts, :key, "dusk/cluster/**")
    ...
    if available?() and live do
      case open_session(connect, mode) do
        {:ok, session} ->
          ...
          Logger.info("Dusk.Zenoh client → #{connect} key=#{key}")
      ...
      Logger.warning("Dusk.Zenoh stub (zenohex=#{available?()} live=#{live} connect=#{connect})")
